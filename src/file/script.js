...
function checkBrowserSupport() {
    if (!('showDirectoryPicker' in window)) {
        showNotification('⚠️ 您的浏览器不支持文件系统访问API\n请使用Chrome、Edge或其他支持的现代浏览器', 'error', 5000);
        return false;
    }
    return true;
}
...
clearFolderBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        title: '清除文件夹记忆',
        message: '确定要清除文件夹记忆吗？',
        details: '这将关闭当前文件夹，下次打开页面将不会自动加载。',
        icon: '🔄',
        type: 'warning',
        confirmText: '继续',
        cancelText: '取消'
        ...
        const secondConfirm = await showConfirmDialog({
            title: '⚠️ 二次确认',
            message: '真的要清除文件夹记忆吗？',
            details: '<strong>此操作将清除：</strong><br>• 文件夹访问权限<br>• 完整路径设置<br>• 每页显示数量等配置',
            icon: '⚠️',
            type: 'danger',
            confirmText: '确认清除',
            cancelText: '我再想想'
...
function showConfirmDialog(options = {}) {
    return new Promise((resolve) => {
        ...
        modalContent.className = 'modal-content confirm-modal-content ' + type;
        
        // 设置按钮文本（使用更可靠的方法）
        const okBtnText = confirmOkBtn.querySelector('.btn-text');
        const cancelBtnText = confirmCancelBtn.querySelector('.btn-text');
        if (okBtnText) okBtnText.textContent = confirmText;
        if (cancelBtnText) cancelBtnText.textContent = cancelText;
        ...
        // ESC键关闭（只在确认对话框打开且其他模态框未打开时响应）
        const handleEscape = (e) => {
            if (e.key === 'Escape' && confirmModal.classList.contains('active')) {
                // 检查是否有其他模态框打开
                const otherModalsOpen = renameModal.classList.contains('active') ||
                                       createFolderModal.classList.contains('active') ||
                                       previewModal.classList.contains('active') ||
                                       setPathModal.classList.contains('active');
                
                // 只有在没有其他模态框打开时才关闭确认对话框
                if (!otherModalsOpen) {
...
selectFolderBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        try {
            ...
            if (permission !== 'granted') {
                showNotification('⚠️ 需要授予文件夹访问权限才能使用文件管理器功能', 'warning', 4000);
                return;
            }
        } catch (err) {
        ...
        if (error.name !== 'AbortError') {
            ...
            showNotification('❌ 选择文件夹失败，请重试', 'error');
        }
    }
});
...
async function addFileToDirectory(file) {
    try {
        ...
        if (fileExists) {
            // 文件已存在，询问用户是否覆盖
            const confirmOverwrite = await showConfirmDialog({
                title: '文件已存在',
                message: `文件 "${file.name}" 已存在`,
                details: '<strong>选择操作：</strong><br>• 确认：覆盖现有文件<br>• 取消：跳过此文件',
                icon: '📄',
                type: 'warning',
                confirmText: '覆盖',
                cancelText: '跳过'
...
confirmCreateFolderBtn.addEventListener('click', async () => {
    ...
    if (!folderName) {
        showNotification('⚠️ 文件夹名称不能为空', 'warning');
        return;
    }
    
    // 检查文件夹名称是否包含非法字符
    if (/[<>:"/\\|?*]/.test(folderName)) {
        showNotification('⚠️ 文件夹名称包含非法字符（< > : " / \\ | ? *）', 'warning');
        return;
    }
...
cancelCreateFolderBtn.addEventListener('click', () => {
    createFolderModal.classList.remove('active');
    newFolderNameInput.value = '';  // 清空输入框
});
...
clearPageBtn.addEventListener('click', async () => {
    ...
    // 构建详细信息
    let detailsHTML = `<strong>本页共 ${itemCount} 项：</strong><br>`;
    if (folderCount > 0) {
        detailsHTML += `• ${folderCount} 个文件夹`;
        if (fileCount > 0) {
            detailsHTML += `<br>• ${fileCount} 个文件`;
        }
    } else {
        detailsHTML += `• ${fileCount} 个文件`;
    }
    
    if (folderCount > 0) {
        detailsHTML += '<br><br><span style="color: #ef4444;">⚠️ 文件夹中的所有内容也将被删除！</span>';
    }
    
    // 第一次确认
    const firstConfirm = await showConfirmDialog({
        title: '清空本页',
        message: `确定要删除本页的 ${itemCount} 项内容吗？`,
        details: detailsHTML,
        icon: '🗑️',
        type: 'warning',
        confirmText: '移到回收站',
    ...
    const secondConfirm = await showConfirmDialog({
        title: '⚠️ 二次确认',
        message: '真的要删除这些内容吗？',
        details: `<strong>即将删除 ${itemCount} 项：</strong><br>` +
                (folderCount > 0 ? `• ${folderCount} 个文件夹<br>` : '') +
                (fileCount > 0 ? `• ${fileCount} 个文件<br>` : '') +
                '<br>删除的文件将移动到回收站。',
        icon: '⚠️',
        type: 'danger',
        confirmText: '确认删除',
...
window.downloadFileByPath = async function(path) {
    ...
    try {
        ...
        // 检查文件大小（防止超大文件导致浏览器崩溃）
        if (fileObj.size > MAX_FILE_SIZE) {
            const fileSize = formatFileSizeForDisplay(fileObj.size);
            const maxSize = formatFileSizeForDisplay(MAX_FILE_SIZE);
            showNotification(
        ...
        try {
            ...
            // 显示保存文件对话框，让用户选择位置和文件名
            const saveFileHandle = await window.showSaveFilePicker(options);
            
            // 写入文件
            showLoading('正在保存文件...', `保存到: ${saveFileHandle.name}`);
            const writable = await saveFileHandle.createWritable();
            await writable.write(fileObj);
            await writable.close();
            
            hideLoading();
            showNotification(`✅ 文件已保存: ${saveFileHandle.name}`, 'success');
            console.log('[下载] 文件已保存:', saveFileHandle.name);
        } catch (error) {
            hideLoading();
            // 用户取消选择或其他错误
            if (error.name === 'AbortError') {
                console.log('[下载] 用户取消了保存');
                showNotification('ℹ️ 已取消保存', 'info');
            } else {
                throw error;
            }
        }
...
confirmRenameBtn.addEventListener('click', async () => {
    ...
    if (!newName) {
        showNotification('⚠️ 名称不能为空', 'warning');
        return;
    }
    ...
    // 检查文件夹名称是否包含非法字符
    if (/[<>:"/\\|?*]/.test(newName)) {
        showNotification('⚠️ 名称包含非法字符（< > : " / \\ | ? *）', 'warning');
        return;
    }
    ...
    try {
        if (currentRenameType === 'file') {
            ...
            try {
                ...
                showNotification(`⚠️ 文件名 "${finalName}" 已存在，请使用其他名称`, 'warning');
                isRenaming = false;  // 释放锁
                isOperating = false;  // 释放锁
                return;
            } catch (e) {
            ...
            if (newName === TRASH_FOLDER_NAME && !currentPath) {
                showNotification('⚠️ 此名称为系统保留，请使用其他名称', 'warning');
                isRenaming = false;  // 释放锁
                isOperating = false;  // 释放锁
                return;
            }
            ...
            try {
                ...
                showNotification(`⚠️ 文件夹名 "${newName}" 已存在，请使用其他名称`, 'warning');
                isRenaming = false;  // 释放锁
                isOperating = false;  // 释放锁
                return;
            } catch (e) {
            ...
            // 确认操作
            const confirmRename = await showConfirmDialog({
                title: '文件夹重命名',
                message: '文件夹重命名需要复制所有内容',
                details: `<strong>原名称：</strong>${item.name}<br><strong>新名称：</strong>${newName}<br><br><span style="color: #f59e0b;">⏱️ 可能需要较长时间，请耐心等待</span>`,
                icon: '📁',
                type: 'warning',
                confirmText: '开始重命名',
                cancelText: '取消'
            ...
            try {
                ...
                if (result.errorCount > 0) {
                    hideLoading();
                    // 复制过程有错误，询问是否继续
                    const continueDelete = await showConfirmDialog({
                        title: '⚠️ 复制过程出现错误',
                        message: `复制过程中有 ${result.errorCount} 个项目失败`,
                        details: `<strong>成功：</strong>${result.copiedCount} 项<br><strong>失败：</strong>${result.errorCount} 项<br><br>是否仍要删除旧文件夹 "${item.name}"？<br><span style="color: #ef4444;">选择"取消"将保留旧文件夹</span>`,
                        icon: '⚠️',
                        type: 'warning',
                        confirmText: '删除旧文件夹',
                        cancelText: '保留旧文件夹'
...
window.deleteItemByPath = async function(path, type) {
    ...
    const itemIcon = type === 'folder' ? '📁' : '📄';
    
    // 构建详细信息
    let detailsHTML = `<strong>名称：</strong>${name}<br>`;
    detailsHTML += `<strong>类型：</strong>${itemType}<br>`;
    if (type === 'folder') {
        detailsHTML += '<br><span style="color: #ef4444;">⚠️ 文件夹中的所有内容也将一起移动</span>';
    }
    ...
    const secondConfirm = await showConfirmDialog({
        title: '⚠️ 二次确认',
        message: `真的要删除 "${name}" 吗？`,
        details: `<strong>操作说明：</strong><br>• 文件将移动到回收站<br>• 可以从回收站恢复`,
        icon: '⚠️',
        type: 'danger',
        confirmText: '确认删除',
        cancelText: '我再想想'
    ...
    try {
        ...
        if (moved) {
            ...
            hideLoading();
            // 移动失败，询问是否永久删除
            const permanentDelete = await showConfirmDialog({
                title: '⚠️ 移动到回收站失败',
                message: '是否要永久删除？',
                details: `<strong>文件：</strong>${name}<br><br><span style="color: #ef4444; font-weight: 600;">⚠️ 警告：此操作不可恢复！<br>文件将被永久删除，无法找回！</span>`,
                icon: '🔥',
                type: 'danger',
                confirmText: '永久删除',
                cancelText: '取消删除'
